/// CRC-24Q (Qualcomm) для RTCM3.
pub struct Crc24q;

impl Crc24q {
    pub const CRC24Q_TAB: [u32; 256] = [
    ...
    /// Считает CRC-24Q по первым `length` байтам `data`, начиная с `crc`.
    pub fn calculate(data: &[u8], length: usize, mut crc: u32) -> u32 {
        debug_assert!(length <= data.len());
        for i in 0..length {
            crc = ((crc << 8) & 0xFFFFFF)
                ^ Self::CRC24Q_TAB[((crc >> 16) as usize) ^ (data[i] as usize)];
        }
        crc
    }
}
...
mod tests {
    ...
    #[test]
    fn empty_input_keeps_initial_crc() {
        assert_eq!(Crc24q::calculate(&[], 0, 0), 0);
        assert_eq!(Crc24q::calculate(&[0xD3], 0, 0xABC), 0xABC);
    }

    #[test]
    fn known_preamble_only_header() {
        // Заголовок пустого кадра: D3 00 00
        let data = [0xD3, 0x00, 0x00];
        let crc = Crc24q::calculate(&data, data.len(), 0);
        assert_eq!(crc, 0x47_EA_4B);
    }
}
